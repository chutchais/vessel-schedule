...
export async function POST(_request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const existing = await prisma.organizationInvitation.findUnique({ where: { id } });
    ...
    const replacement = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.organizationInvitation.create({ data: { organizationId: existing.organizationId, email: existing.email, role: existing.role, pendingKey: `${existing.organizationId}:${existing.email}`, tokenHash, expiresAt: getInvitationExpiry(now), invitedById: currentUser.id } });
    ...
    return NextResponse.json({ data: { id: replacement.id, expiresAt: replacement.expiresAt, invitationUrl: buildInvitationUrl(token) } });
  } catch (error) {
